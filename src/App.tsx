...
interface Feedback { id: number | string; name: string; rating: number; comment: string; approved: boolean; date: string; }
...
function App() {
  ...
  const fetchRate = async () => {
    try {
      ...
      console.log('Using default rate (API unavailable)');
    }
  };
  ...
  const fetchInquiries = async () => {
    try {
      ...
      console.log('Could not fetch inquiries');
    }
  };
  ...
  const fetchFeedbacks = async () => {
    try {
      ...
      console.log('Could not fetch feedbacks');
    }
  };
  ...
  const calculateEstimate = () => {
    ...
    toast.success(`Estimate of ₹${cost.toLocaleString('en-IN')} calculated using live rate!`);
  };
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // refresh inquiries list if admin is watching
        if (isAdminLoggedIn) fetchInquiries();
      } else {
        toast.error('Failed to submit. Please try again.');
      }
    } catch (err) {
      toast.success('Inquiry received (Demo Mode)');
      setContactForm({ name: '', phone: '', email: '', service: 'With Material', message: '' });
    } finally {
      setLoading(false);
    }
  ...
  const handleFeedbackSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/feedback`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...feedbackForm, approved: false }),
      });
      if (res.ok) {
        toast.success('Thank you! Your feedback is awaiting admin approval.');
  ...
  const approveFeedback = async (id: number | string) => {
    try {
      ...
      fetchFeedbacks();
      toast.success('Feedback approved and now visible to visitors!');
    } catch (err) {
  ...
  const updateRate = async () => {
    ...
    try {
      ...
      toast.success('Rate updated (Demo Mode)');
    }
  };
  ...
  const addToGallery = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setGalleryItems(prev => [{ id: newItem._id || Date.now(), url: newItem.url, caption: newItem.caption, category: newItem.category }, ...prev]);
        toast.success('Project added to gallery!');
      }
    } catch (err) {
      setGalleryItems(prev => [{ id: Date.now(), url: '/images/house1.jpg', caption: newGalleryCaption, category: newGalleryCategory }, ...prev]);
  ...
  // ─── Admin login — credentials match the modal's demo hint ──────────────────
  const handleAdminLogin = () => {
    if (adminEmail.toLowerCase() === ADMIN_EMAIL.toLowerCase() && adminPassword === ADMIN_PASSWORD) {
      ...
      fetchInquiries(); // load inquiries now that admin is in
    } else {
      setAdminLoginError(`Invalid credentials. Use ${ADMIN_EMAIL} / ${ADMIN_PASSWORD}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-gray-800">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-4">
              ...
              <a href="https://wa.me/919525185568" target="_blank" rel="noreferrer" className="hidden sm:flex items-center gap-2 px-6 py-3 bg-[#25D366] text-white rounded-3xl text-sm font-medium">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white px-6 py-8 border-t text-lg flex flex-col gap-6">
            {['services', 'packages', 'calculator', 'gallery', 'testimonials', 'contact'].map(section => (
              <button key={section} onClick={() => scrollToSection(section)} className="text-left capitalize">{section}</button>
            ))}
      ...
      {/* FLOATING BUTTONS */}
      <a href="https://wa.me/919525185568" target="_blank" rel="noreferrer" className="floating-btn whatsapp-btn text-white">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-12">
            <div className="max-w-xs mx-auto">
              ...
              <div className="space-y-8">
                ...
                <button onClick={calculateEstimate} className="w-full h-16 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-3xl text-xl active:scale-[0.985] transition-all shadow-inner">
                  CALCULATE ESTIMATE
                </button>
                {estimatedCost > 0 && (
                  <div className="bg-white border border-emerald-200 rounded-3xl p-8 text-center">
                    ...
                    <div className="text-xs text-gray-400 mt-2">Approximate figure. Final quote after site visit.</div>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="mt-20 max-w-xl mx-auto bg-white shadow rounded-3xl p-10">
            ...
            <form onSubmit={handleFeedbackSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map(rating => (
                    <button key={rating} type="button" onClick={() => setFeedbackForm({ ...feedbackForm, rating })} className="star text-4xl transition-all hover:scale-110">
              ...
              <textarea
                ...
                required
              />
              <button type="submit" disabled={loading} className="w-full py-5 bg-gray-900 hover:bg-black text-white font-medium rounded-3xl flex items-center justify-center gap-3">
        ...
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-5 gap-16">
          <div className="md:col-span-2">
            <div className="sticky top-28">
              ...
              <div className="mt-8 space-y-6 text-slate-300">
                <div className="flex gap-4"><div className="w-6">📍</div><div>Tej Pratap Nagar, Beur,<br />Patna-2</div></div>
                <div className="flex gap-4"><div className="w-6">☎</div><div>+91 95251 85568</div></div>
                <div className="flex gap-4"><div className="w-6">✉</div><div>ramchhotan63@gmail.com</div></div>
              </div>
              <div className="mt-16 text-xs leading-loose text-slate-500">All inquiries responded within 4 hours during business days.</div>
            </div>
          </div>
          <div className="md:col-span-3">
            <form onSubmit={handleContactSubmit} className="bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-3xl">
              ...
              <textarea value={contactForm.message} onChange={e => setContactForm({ ...contactForm, message: e.target.value })} placeholder="Tell us about your project..." rows={5} className="mt-6 w-full bg-white/10 border border-white/30 rounded-3xl px-7 py-6 placeholder:text-slate-400" required />
              <button type="submit" disabled={loading} className="mt-8 w-full py-6 bg-white text-gray-900 font-semibold rounded-3xl flex items-center justify-center gap-3 hover:bg-amber-200">
                SEND INQUIRY <Send size={19} />
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-12 gap-y-16">
          ...
          <div className="md:col-span-3 text-sm">
            ...
            <div className="space-y-4">
              {['services', 'packages', 'gallery'].map(s => (
                <div key={s} onClick={() => scrollToSection(s)} className="cursor-pointer hover:text-white capitalize">{s}</div>
              ))}
            </div>
          </div>
          <div className="md:col-span-4">
            <div className="uppercase text-xs text-white/40 mb-5">OFFICE</div>
            <div className="text-sm leading-loose">Tej Pratap Nagar, Beur,<br />Patna, Bihar - 800002<br /><br />+91 95251 85568</div>
        ...
        <div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-6">
          <div className="bg-white rounded-3xl w-full max-w-md p-10 shadow-2xl">
            ...
            <div className="space-y-6">
              ...
              {adminLoginError && (
                <div className="text-red-600 text-sm text-center bg-red-50 py-3 rounded-2xl">{adminLoginError}</div>
              )}
              <button onClick={handleAdminLogin} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-2xl text-lg transition-all active:scale-95">
                LOGIN TO ADMIN PANEL
              </button>
              {/* Demo hint — matches actual credentials above */}
              <div className="text-center text-xs text-gray-400">
                Demo credentials:<br />
                <span className="font-mono text-blue-600">{ADMIN_EMAIL}</span><br />
                Password: <span className="font-mono">{ADMIN_PASSWORD}</span>
              </div>
              <button onClick={() => setShowAdminLogin(false)} className="w-full text-gray-500 hover:text-gray-700 text-sm py-2">Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/90 z-[999] flex overflow-hidden">
          <div className="admin-sidebar w-72 text-white p-8 flex-shrink-0 overflow-auto">
            <div className="flex items-center justify-between mb-12">
              ...
              <button onClick={adminLogout} className="p-2 hover:bg-white/10 rounded-xl transition"><LogOut size={24} /></button>
            </div>
              ...
              {[
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    if (item.id === 'inquiries') fetchInquiries();
                  }}
                  className={`px-6 py-4 rounded-2xl cursor-pointer font-medium transition-all ${activeAdminTab === item.id ? 'bg-white text-gray-900 shadow' : 'hover:bg-white/10'}`}
                >
                  {item.label}
                </div>
              ))}
            </div>
            <div className="absolute bottom-8 left-8 text-xs opacity-50">Logged in as<br />{ADMIN_EMAIL}</div>
          </div>
            ...
            <div className="max-w-6xl mx-auto">
              ...
              {activeAdminTab === 'dashboard' && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-10">
                    <div className="bg-white rounded-3xl p-8 shadow"><div className="text-sm text-gray-500">TOTAL INQUIRIES</div><div className="text-6xl font-semibold text-blue-600 mt-4">{inquiries.length}</div></div>
                    <div className="bg-white rounded-3xl p-8 shadow"><div className="text-sm text-gray-500">APPROVED REVIEWS</div><div className="text-6xl font-semibold text-emerald-600 mt-4">{approvedFeedbacks.length}</div></div>
                    <div className="bg-white rounded-3xl p-8 shadow"><div className="text-sm text-gray-500">CURRENT RATE</div><div className="text-6xl font-semibold text-amber-600 mt-4">₹{currentRate}</div></div>
                    <div className="bg-white rounded-3xl p-8 shadow"><div className="text-sm text-gray-500">GALLERY ITEMS</div><div className="text-6xl font-semibold text-purple-600 mt-4">{galleryItems.length}</div></div>
                  </div>
                </div>
              )}
                    ...
                    {inquiries.length === 0 ? (
                      ...
                      <div key={inq.id ?? inq._id} className="bg-white border rounded-3xl p-8 flex gap-8">
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-500">{inq.phone} • {inq.date}</div>
              ...
              {activeAdminTab === 'rate' && (
                <div className="max-w-lg mx-auto">
                  ...
                  <p className="text-gray-600 mb-10">This rate is used in the Cost Calculator.</p>
                  <div className="bg-white rounded-3xl p-12 border">
                    ...
                    <input type="number" value={newRate} onChange={(e) => setNewRate(Number(e.target.value))} className="w-full text-center text-7xl font-light border-b-4 border-gray-200 focus:border-blue-500 outline-none py-6" />
                    <button onClick={updateRate} className="mt-12 w-full py-5 bg-black text-white font-medium rounded-2xl text-lg hover:bg-gray-800 transition">UPDATE RATE</button>
                  </div>
                </div>
              )}
              ...
              {activeAdminTab === 'gallery' && (
                <div>
                  <div className="flex justify-between items-center mb-10">
                    ...
                    <button onClick={() => setShowAddGallery(true)} className="flex items-center gap-2 bg-black text-white px-8 py-4 rounded-3xl text-sm font-medium">+ Add New Project</button>
                  </div>
                    ...
                    {galleryItems.map(item => (
                      <div key={item.id} className="bg-white rounded-3xl overflow-hidden border">
                        ...
                        <div className="p-6">
                          ...
                          <div className="font-medium mt-2">{item.caption}</div>
                          <button onClick={() => deleteGalleryItem(item.id)} className="mt-6 text-red-600 text-sm flex items-center gap-2 hover:text-red-700"><Trash2 size={16} /> Delete Project</button>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={fb.id} className={`bg-white border rounded-3xl p-8 ${fb.approved ? 'border-emerald-200 bg-emerald-50' : ''}`}>
                        <div className="flex justify-between">
                          <div>
                            ...
                            <div className="flex gap-1 mt-2">{Array.from({ length: 5 }).map((_, i) => <span key={i} className={i < fb.rating ? 'text-amber-400' : 'text-gray-200'}>★</span>)}</div>
                          </div>
                        ...
                        <div className="flex gap-4 mt-8">
                          {!fb.approved && (
                            <button onClick={() => approveFeedback(fb.id)} className="px-8 py-3 bg-emerald-600 text-white rounded-2xl text-sm font-medium">Approve Feedback</button>
                          )}
                          <button onClick={() => deleteFeedback(fb.id)} className="px-8 py-3 border border-red-300 text-red-600 rounded-2xl text-sm font-medium hover:bg-red-50">Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-[9999] flex items-center justify-center p-6">
          <div className="bg-white rounded-3xl max-w-lg w-full p-10">
            ...
            <div className="mb-8">
              ...
              <input value={newGalleryCaption} onChange={(e) => setNewGalleryCaption(e.target.value)} className="w-full border-2 rounded-2xl px-6 py-5" placeholder="E.g. 2400 sqft villa in Danapur" />
            </div>
            <div>
              <div className="text-xs font-medium mb-3 text-gray-500">CATEGORY</div>
              <div className="flex gap-3">
                {['completed', 'ongoing', 'interior'].map(cat => (
                  <button key={cat} onClick={() => setNewGalleryCategory(cat)} className={`flex-1 py-4 text-sm rounded-2xl border ${newGalleryCategory === cat ? 'border-black bg-black text-white' : 'hover:bg-gray-100'}`}>{cat.toUpperCase()}</button>
                ))}
              </div>
            </div>
      ...
      {isAdminLoggedIn && (
        <div className="fixed bottom-4 left-4 bg-amber-100 border border-amber-300 text-amber-800 text-xs px-4 py-2.5 rounded-2xl z-[9999] max-w-[260px]">
          Admin Mode • Use sidebar logout to exit
        </div>
      )}
    </div>
  );
